...
# 6. Check the user's role from the database (by email) after the second login
db = SessionLocal()
try:
    user = db.query(User).filter(User.email == email).first()
    if user:
        print(f"User role from DB after second login: {user.role}")
    else:
        print("User not found after second login")
finally:
    db.close()
